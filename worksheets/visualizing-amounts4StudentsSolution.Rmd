...
```{r geom-col-sorted, exercise=TRUE}
ggplot(boxoffice, aes(x = amount, y = fct_reorder(title, amount), fill = amount)) +
  geom_col() +
  xlab("weekend gross (million USD)") +
  ylab(NULL)
```
...
```{r geom-col-sorted, exercise=TRUE}
ggplot(boxoffice, aes(x = amount, y = fct_reorder(title, amount), fill = title)) +
  geom_col() +
  xlab("weekend gross (million USD)") +
  ylab(NULL)
```
...
```{r geom-bar, exercise=TRUE}
ggplot(penguins_nomissing, aes(y = species)) +
  geom_bar()
```
...
```
```{r geom-bar, exercise=TRUE}
ggplot(penguins_nomissing, aes(y = fct_relevel(species, "Chinstrap", "Gentoo", "Adelie"))) +
  geom_bar() + 
  ylab(NULL)
```
